...
def saveFile(name, array):
    f = open("Result/Objetos/"+name+".cpt", "w")
    for i in array:
...
def saveFileFullPts(path, name, array, w):
    ...
    for i in range(len(array[0])):
        for j in range(len(array)):
            f.write(str(array[j][i][0])+','+str(array[j][i][1])+','+str(array[j][i][2])+','+str(w[j]))
            if j<len(array)-1:
                f.write(";")
        f.write("\n")
    f.close()
...
def loadFile(pathes):
    ...
    for i in pathes:
        ...
        print(str(pathes.index(i))+'/'+str(np.size(pathes))+'\t'+i+'\t'+str(time.time()-ini)+'\t'+str(time.time()-start_time))
    return imgs
...
def saveImagesSided(image, folder, processo, onerow=True):
    pathes = []
    if onerow:
        newimg = np.concatenate((image[2], image[1], image[0], image[3], image[4]), axis=1)
        ...
        newimg = np.vstack((newimg, np.zeros(imgs[0].shape), np.zeros(imgs[0].shape)))
...
def showPoints(points, ref, ell):
    ...
    arr = ['frontal', 'obliquo direito(45)', 'direito total(90)', 'obliquo esquerdo(-45)', 'esquerdo total(-90)']
...
def cannyImg(images, th1, th2):
    ...
    for i in range(len(images)):
       ...
       print('Canny:'+'\t'+str(i)+'\t'+str(time.time()-ini)+'\t'+str(time.time()-start_time))
    return newimg
...
def drawPoints(points, size, color, img=None):
    ...
    for i in range(len(points)):
       ...
       for j in range(len(pts)-1):
           cv2.line(auximg, ((int)(pts[j][0]), (int)(pts[j][1])), ((int)(pts[j+1][0]), (int)(pts[j+1][1])), color, 3)
       cv2.line(auximg, ((int)(pts[len(pts)-1][0]), (int)(pts[len(pts)-1][1])), ((int)(pts[0][0]), (int)(pts[0][1])), color, 3)
       newimg.append(auximg)
       #newimg.append(cv2.drawContours(np.zeros((images[i].shape[0],images[i].shape[1],3), np.uint8), c, -1, (0,255,0), 3))
       #print('Contorno:'+'\t'+str(i)+'\t'+str(time.time()-ini)+'\t'+str(time.time()-start_time))
    return newimg
...
def perspectivePts(points):
    for i in range(len(points)):
        for j in range(len(points[i])):
            if points[i][j][3] == 0:
                points[i][j] = [points[i][j][0],points[i][j][1], points[i][j][2]]
            else:
                points[i][j] = [points[i][j][0],points[i][j][1], points[i][j][2]]
...
def anisotropicFilter(img, n, k, g):
    ...
    for i in range(len(img)):
       ...
       newimg.append(anisotropic_diffusion(imgs[i], niter=n, kappa=k, gamma=g, option=2))
       #print('Contorno:'+'\t'+str(i)+'\t'+str(time.time()-ini)+'\t'+str(time.time()-start_time))
...
def showSurface(path,name,u,v, texturepath):
    ...
    # Set control points
    surf.set_ctrlpts(*exchange.import_txt(path+name+"_ctrlpoints.cptw", two_dimensional=True))
    ...
    for i in surf.faces:
        for j in i.vertices:
            ...
            color = [1.0, 1.0, 1.0];
            tx = [j[0]/256, j[1]/256]
            result = result+str(j[0])+' '+str(j[1])+' '+str(j[2])+' '+str(color[0])+' '+str(color[1])+' '+str(color[2])+' '+str(tx[0])+' '+str(tx[1])+' '
...
def superpixel(images):
    ...
    for i in range(len(images)):
       ini = time.time()
       newimg.append(mark_boundaries(images[i], felzenszwalb(images[i], scale=200, sigma=0.5, min_size=480)))
    return newimg
...
def surf(images):
    ...
    for i in range(len(images)):
        ...
        newimg.append(cv2.drawKeypoints(images[i].astype(np.uint8), kp, None, color=(0,255,0), flags=0))
